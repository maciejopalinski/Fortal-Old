...
class Lexer
{
    ...
    void saveState() { saved_location = Location(location); }
    ...
    bool eat(string expected, bool report_error = false)
    {
        ...
        else if (report_error)
        {
            ...
            error_handler.log_unexpected_token(location, expected, unexpected);
        }
        return false;
    }
    ...
    bool expect(string compare1, bool whitespace_after = false)
    {
        ...
        if (whitespace_after)
        {
            ...
            result &=
                (isWhitespace(c) || isSeparator(c) || isBracket(c) || isNewline(c) || isNULL(c));
        }
        return result;
    }
